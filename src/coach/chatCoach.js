    ...
    const fetchClientProfiles = async (clientIds) => {
      ...
      await Promise.all(
        clientIds.map(async (clientId) => {
          if (!clientsInfo[clientId]) {  // Fetch only if not already fetched
            const clientDoc = await getDoc(doc(db, 'users', clientId));
            if (clientDoc.exists()) {
              const clientData = clientDoc.data();
              newClientsInfo[clientId] = {
                email: clientData.email, // Store email instead of name
                profilePic: clientData.fileURL || 'default-client.png', // Fallback image
              };
            }
          }
        })
          ...
          return (
            <div key={chat.id} className="chat-box">
              <img
                ...
                alt={`${clientInfo.email}'s Profile`} // Use email as the alt text
